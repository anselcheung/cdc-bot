...
class handler(CDCAbstract):
    def __init__(
        ...
        self.platform = (
            ...
            else "windows" if "win32" in sys.platform else "osx"
        )
        ...
        options = (
            browser_type.lower() == "firefox"
            and webdriver.FirefoxOptions()
            or webdriver.ChromeOptions()
        )
        if headless:
        ...
        driver_name = (
            "geckodriver" if browser_type.lower() == "firefox" else "chromedriver"
        )
        ...
        super().__init__(
            username=self.username, password=self.password, headless=headless
        )
    ...
    def get_earliest_time_slots(
        ...
        sorted_datetimes.sort(
            key=lambda comp_date: convert_to_datetime(comp_date[0], comp_date[1])
        )

        return_sessions_data = {}
        step = (
            2 if field_type == Types.SIMULATOR else 1
        )  # No back-to-back sessions allowed for simulator lessons
    ...
    def dismiss_normal_captcha(
        self,
        caller_identifier: str,
        solve_captcha: bool = False,
        secondary_alert_timeout: int = 5,
        force_enabled: bool = False,
    ):
        is_captcha_present = selenium_common.is_elem_present(
        ...
        if solve_captcha:
            success, _ = self.captcha_solver.solve(
                driver=self.driver,
                captcha_type="normal_captcha",
                force_enable=force_enabled,
            )
            if not success:
                return False

            captcha_submit_btn = selenium_common.wait_for_elem(
                self.driver, By.ID, "ctl00_ContentPlaceHolder1_Button1"
            )
            captcha_submit_btn.click()
        else:
            captcha_close_btn = selenium_common.wait_for_elem(
                self.driver, By.CLASS_NAME, "close"
            )
        ...
        if "incorrect captcha" in alert_text:
            selenium_common.dismiss_alert(
                driver=self.driver, timeout=secondary_alert_timeout
            )
            self.log.info(
                f"Normal captcha failed for opening {caller_identifier} page."
            )
            return False
    ...
    def accept_terms_and_conditions(self):
        terms_checkbox = selenium_common.is_elem_present(
            self.driver, By.ID, "ctl00_ContentPlaceHolder1_chkTermsAndCond"
        )
        agree_btn = selenium_common.is_elem_present(
    ...
    def select_course_from_idx(self, course_data: Dict, course_idx: int):
        if course_idx < 0 or course_idx > len(course_data["available_courses"]):
            self.log.error(
                f"Course selected is out of range. {course_data['available_courses']}"
            )
            return False
    ...
    def open_home_page(self, sleep_delay: Union[int, None] = None):
        ...
        if "ComfortDelGro" not in self.driver.title:
            success, _ = self.captcha_solver.cloudflare_turnstile(self.driver)
        if sleep_delay:
            time.sleep(sleep_delay)
    ...
    def account_login(self):
        ...
        # prompt_login_btn = selenium_common.wait_for_elem(
        #     self.driver, By.XPATH, "//*[@id='top-menu']/ul/li[10]/a"
        # )
        prompt_login_btn = selenium_common.wait_for_elem(
            self.driver, By.XPATH, '//*[@id="login"]/div/div/span'
        )
        prompt_login_btn.click()

        learner_id_input = selenium_common.wait_for_elem(
            self.driver, By.NAME, "userId_4"
        )
        password_input = selenium_common.wait_for_elem(
            self.driver, By.NAME, "password_4"
        )
        ...
        success, _ = self.captcha_solver.solve(
            driver=self.driver, captcha_type="recaptcha_v2"
        )
        if success:
            login_btn = selenium_common.wait_for_elem(
                self.driver, By.XPATH, '//*[@id="login-form"]/div[4]/input'
            )
            login_btn.click()
    ...
    def get_reserved_lesson_date_time(self):
        rows = self.driver.find_elements(
            By.CSS_SELECTOR, "table#ctl00_ContentPlaceHolder1_gvReserved tr"
        )
        ...
        for row in rows:
            ...
            if len(td_cells) > 0:
                ...
                field_type = (
                    Types.SIMULATOR
                    if "SIMULATOR" in lesson_name
                    else (
                        Types.PRACTICAL
                        if "AUTOCAR" in lesson_name
                        else (
                            Types.PRACTICAL
                            if "2BL" in lesson_name
                            else (
                                Types.PRACTICAL
                                if "ONETEAM" in lesson_name
                                else (
                                    Types.BTT
                ...
                if field_type and field_type != Types.PRACTICAL:
                    self.set_attribute_with_fieldtype(
                        "lesson_name", field_type, lesson_name
                    )
                    reserved_sessions = self.get_attribute_with_fieldtype(
                        "reserved_sessions", field_type
                    )
                    ...
                    if td_cells[0].text not in reserved_sessions:
                        reserved_sessions.update(
                            {
                                td_cells[0].text: [
                                    f"{td_cells[2].text[:-3]} - {td_cells[3].text[:-3]}"
                                ]
                            }
                        )
                    else:
                        reserved_sessions[td_cells[0].text].append(
                            f"{td_cells[2].text[:-3]} - {td_cells[3].text[:-3]}"
                        )

    def get_booked_lesson_date_time(self):
        rows = self.driver.find_elements(
            By.CSS_SELECTOR, "table#ctl00_ContentPlaceHolder1_gvBooked tr"
        )
        ...
        for row in rows:
            ...
            if len(td_cells) > 0:
                ...
                if field_type:
                    ...
                    booked_sessions = self.get_attribute_with_fieldtype(
                        "booked_sessions", field_type
                    )
                    ...
                    if td_cells[0].text not in booked_sessions:
                        ...
                        booked_sessions[td_cells[0].text].append(
                            f"{td_cells[2].text[:-3]} - {td_cells[3].text[:-3]}"
                        )
    ...
    def open_theory_test_booking_page(self, field_type: str, call_depth: int = 0):
        ...
        if not self.check_access_rights("NewPortal/Booking/BookingTT.aspx"):
            self.log.debug(
                f"User does not have {field_type.upper()} as an available option."
            )
            return False

        if not self.dismiss_normal_captcha(
            caller_identifier=f"{field_type.upper()} Booking", solve_captcha=False
        ):
        ...
        test_name_element = selenium_common.wait_for_elem(
    ...
    def open_practical_lessons_booking_page(self, field_type: str, call_depth: int = 0):
        ...
        if not course_data:
            self.log.error(
                "Could not get course data. Program probably encountered a hCaptcha."
            )
            raise Exception(
                "Could not get course data. Program probably encountered a hCaptcha."
            )
        ...
        if not (
            self.select_course_from_name(course_data, "Class 3A Motorcar")
            or self.select_course_from_idx(course_data, 1)
        ):
            self.log.warning("Could not a select course.")
            return False

        if not self.dismiss_normal_captcha(
            caller_identifier="Practical Lessons Booking", solve_captcha=True
        ):
        ...
        if selenium_common.is_elem_present(
            ...
            self.notification_manager.send_notification_all(
                title="", msg="No available practical lessons currently"
            )
            return False

        # Check if the user is able to book from other teams
        if selenium_common.is_elem_present(
            self.driver, By.ID, "ctl00_ContentPlaceHolder1_ddlOthTeamID"
        ):
            available_teams = self.get_course_data(
                "ctl00_ContentPlaceHolder1_ddlOthTeamID"
            )

            if (
                self.program_config["book_from_other_teams"]
                and len(available_teams["available_courses"]) > 1
            ):
                ...
                for idx in range(1, len(available_teams["available_courses"])):
                    ...
                    if len(available_teams["available_courses"]) > idx:
                        selected_team = self.select_course_from_idx(
                            available_teams, idx
                        )
                        available_teams_str += "=======================\n"
                        ...
                        loading_element = selenium_common.wait_for_elem(
                            self.driver,
                            By.ID,
                        ...
                        self.get_all_available_sessions(
                            Types.PRACTICAL, team_available_sessions
                        )

                        for (
                            available_date_str,
                            available_time_slots,
                        ) in team_available_sessions.items():
                ...
                self.notification_manager.send_notification_all(
                    title=f"SESSIONS FROM OTHER TEAMS DETECTED", msg=available_teams_str
                )
    ...
    def open_simulator_lessons_booking_page(self, field_type: str, call_depth: int = 0):
        ...
        if not (
            self.select_course_from_name(course_data, "Simulator Course - Car (School)")
            or self.select_course_from_idx(course_data, 1)
        ):
            self.log.warning("Could not a select course.")
            return False
        ...
        if not self.dismiss_normal_captcha(
            caller_identifier="Simulator Lessons Booking", solve_captcha=True
        ):
    ...
    def open_practical_test_booking_page(self, field_type: str, call_depth: int = 0):
        ...
        if not self.dismiss_normal_captcha(
            caller_identifier="Practical Test Booking", solve_captcha=True
        ):
    ...
    def get_all_session_date_times(self, field_type: str):
        for row in self.driver.find_elements(
            ...
            selected_times_array = self.get_attribute_with_fieldtype(
                "times_in_view", field_type
            )
            selected_days_array = self.get_attribute_with_fieldtype(
                "days_in_view", field_type
            )
    ...
    def get_all_available_sessions(self, field_type: str, local_tb: Dict = None):
        ...
        for input_element in input_elements:
            ...
            if any(
                gif in input_element_src
                for gif in ["Images1.gif", "Images2.gif", "Images3.gif"]
            ):
                ...
                web_elements_in_view = (
                    {}
                    if local_tb is not None
                    else self.get_attribute_with_fieldtype(
                ...
                if "Images1.gif" in input_element_src:
                    ...
                    if available_session_date not in available_sessions:
                        available_sessions.update(
                            {available_session_date: [available_session_time]}
                        )
                    elif (
                        available_session_time
                        not in available_sessions[available_session_date]
                    ):
                        available_sessions[available_session_date].append(
                            available_session_time
                        )
        ...
        if (
            last_practical_input_element is None
            or has_booked_lessons_in_view
            or booked_sessions
        ):
            return
        ...
        try:
            self.log.info(
                f"Attempting to reserve a session to check if user can book {field_type.upper()}"
            )
            ...
            self.log.warning(
                f"User can't book {field_type.upper()} because '{alert.text}'"
            )
    ...
    def create_notification_update(self, field_type: str):
        earlier_sessions = self.get_attribute_with_fieldtype(
            "earlier_sessions", field_type
        )
        booked_sessions = self.get_attribute_with_fieldtype(
            "booked_sessions", field_type
        )
    ...
    def update_earlier_sessions(self, field_type: str):
        available_sessions = self.get_attribute_with_fieldtype(
            "available_sessions", field_type
        )
            ...
            for available_date_str, available_time_slots in available_sessions.items():
                ...
                for booked_date_str in booked_sessions:
                    ...
                    if valid_booked_date and available_date_str not in earlier_sessions:
                        earlier_sessions[available_date_str] = list(
                            available_time_slots
                        )

            self.set_attribute_with_fieldtype(
                "earlier_sessions", field_type, dict(earlier_sessions)
            )
        else:
            self.set_attribute_with_fieldtype(
                "earlier_sessions", field_type, dict(available_sessions)
            )

    def flush_notification_update(self):
        if self.notification_update_msg != "":
            self.notification_manager.send_notification_all(
                title=f"{datetime.datetime.now()}", msg=self.notification_update_msg
            )
    ...
    def check_if_earlier_available_sessions(self, field_type: str):
        ...
        web_elements_in_view = self.get_attribute_with_fieldtype(
            "web_elements_in_view", field_type
        )
        ...
        if not self.check_if_same_sessions(
        ...
        if self.auto_reserve and number_of_slots_needed > 0:
            earliest_sessions_to_be_reserved = self.get_earliest_time_slots(
                earlier_sessions, number_of_slots_needed, field_type
            )
            ...
            for reserved_date_str, reserved_time_slots in reserved_sessions.items():
                ...
                for earliest_date_str in earliest_sessions_to_be_reserved:
                    ...
                    for reserved_time_slot in reserved_time_slots:
                        input_element_id = web_elements_in_view[
                            f"{reserved_date_str} : {reserved_time_slot}"
                        ]
                        input_element = selenium_common.wait_for_elem(
                            self.driver, By.ID, input_element_id
                        )
                        input_element.click()
                        ...
                        if alert_found:
                            ...
                            to_be_removed_reservations[reserved_date_str].append(
                                reserved_time_slot
                            )
            ...
            if number_of_slots_needed > 0:
                self.log.info(
                    f"Number of slots to reserve for {field_type.upper()} is: {number_of_slots_needed}"
                )
                earliest_sessions_to_be_reserved = self.get_earliest_time_slots(
                    earlier_sessions, number_of_slots_needed, field_type
                )
                    ...
                    for time_slot in time_slots:
                        ...
                        if alert_found:
                            ...
                            if any(
                                ...
                                "Back to Back session is not allowed" in alert_text
                            ):  # for simulator
                                continue
        ...
        self.set_attribute_with_fieldtype(
            "reserved_sessions", field_type, dict(reserved_sessions)
        )
        self.set_attribute_with_fieldtype(
            "available_sessions", field_type, dict(available_sessions)
        )
        self.update_earlier_sessions(field_type)
        self.set_attribute_with_fieldtype(
            "cached_earlier_sessions",
            field_type,
